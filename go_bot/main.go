...
func main() {
	...
	b, _ := bot.New(os.Getenv("BOT_TOKEN"), opts...)
...
func handlerPaymentCommand(ctx context.Context, b *bot.Bot, update *models.Update) {
	fmt.Printf("handlerPaymentCommand: %+v\n", update)
	m, err := b.SendInvoice(ctx, &bot.SendInvoiceParams{
		...
		Prices: []models.LabeledPrice{
			{Label: "Some label", Amount: 1},
		},
	})
	if err != nil {
		fmt.Printf("error sending invoice: %s\n", err)
	} else {
		fmt.Printf("invoice sent: %+v\n", m)
	}
}
